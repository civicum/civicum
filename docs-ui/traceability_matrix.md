...
| UI-CMP-006 | MUST | Smart Dock: 5 posiciones (pos 5 = "Más"), badge = punto rojo 8px sin número | S01 → §9.1+§9.2 → "Badge: Punto rojo 8px (sin número — reduce ansiedad)" (L1146) | **PARTIAL** | ALTO | E2E + Golden screenshots | **Gate 6:** 5 tabs implementados. Tab 'Más' es overlay funcional via `MoreMenu.tsx`. **Badge: DEFERRED.** Componente visual de badge rojo 8px existe pero está hardcodeado (fijo). No hay suscripción a sistema de notificaciones backend. Cierre real requiere integración con backend de notificaciones (fuera de scope actual). |
...
| UI-LAY-004 | MUST | Smart Dock: bottom bar XS-MD, side rail LG+ | S01 → §6.3 → fila Smart Dock (L635) | OK | ALTO | E2E + Golden screenshots | **Implemented Gate 5.9a (final).** SmartDock `lg:hidden` (visible 360/480/768, hidden 1024+). SideRail.tsx (`fixed left, top-16, w-[72px], lg:flex`). AppLayout usa wrapper compartido `<div lg:pl-[72px]>` envolviendo OfflineBanner + main — no `lg:ml-[72px]` (era defectuoso con `max-w-7xl mx-auto`). Zero overflow horizontal verificado por E2E (`scrollWidth ≤ clientWidth` at 1024px). Oclusión OfflineBanner verificada por bounding-box. Hash de hito/verificación: `6e2ed35`. |
...
| UI_RULE_ID | Tipo | Regla | DOCREF | Estado | Impacto | Verificación | Notas |
|------------|------|-------|--------|--------|---------|-------------|-------|
| UI-NAV-001 | MUST | Dock order: Inicio, Academia, Civia, Alza la Voz, Más | ADR-0009 (S01 §9.1 histórico) | OK | ALTO | E2E | **Cerrado por reconciliación documental (ADR-0009 ACCEPTED).** Taxonomía oficial: Inicio / Academia / Civia / Alza la Voz / Más. Implementada Gate 6 (Ruta C). Redirección `/circulos` → `/alza-la-voz`. Labels alineados con voz de marca (UI-CNT-001/002). Memo: `_reentry/UI_NAV_001_RECONCILIATION_MEMO.md`. |
| UI-NAV-002 | MUST | Dock altura 56px + safe area iOS | S01 → §9.2 → "Altura: 56px" (L1141) | OK | MEDIO | Golden screenshots | **Gate 5.9a (cerrado).** Estructura: `<nav pb-safe>` → `<div h-[56px]>` (zona útil) → items. Total exterior = 56px + env(safe-area-inset-bottom). `viewport-fit=cover` confirmado presente en `webapp/index.html`. CSS utility `.pb-safe { padding-bottom: env(safe-area-inset-bottom) }` en `index.css`. Verificable en dispositivo iOS real con notch; en desktop/emulador el inset es 0px y el dock mide exactamente 56px. Hash de hito/verificación: `6e2ed35`. |
...
| UI-STP-004A | MUST | Error Crítico/Fetch Inicial: Full-screen ErrorState | S01 → §12.1 / ADR-0008 | OK | ALTO | UI-kit Playwright + E2E | Implemented Gate 5.3 (`ErrorState.tsx`). Integrado en producción Gate 5.7 vía Dashboard. Valida fallback estructural tras fallo de DB sin romper layout. |
...
| UI-STP-006 | MUST | offline.html digno con logo + capacidades + CTA | S03 → F-07 → "Dignidad = experiencia completa" (L365) | **NO IMPL** | ALTO | — | **No implementado (confirmado Gate 5.9a audit).** `offline.html` estático no existe. Service Worker ausente. Web App Manifest ausente. La app no tiene capacidad offline real (PWA). OfflineBanner (UI-STP-005) cubre solo detección visual en-app via `navigator.onLine` — no es sustituto de esta regla. Requiere: (1) `offline.html` con logo + CTA, (2) Service Worker para servir fallback, (3) `manifest.json`. Todo fuera de scope actual. No marcar OK hasta implementación real. |
