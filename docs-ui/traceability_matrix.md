...
| UI_RULE_ID | Tipo | Regla | DOCREF | Estado | Impacto | Verificación | Notas |
|------------|------|-------|--------|--------|---------|-------------|-------|
| UI-NAV-001 | MUST | Dock order: Inicio, Academia, Civia, Alza la Voz, Más | Anexo MVP → §2 | OK | ALTO | E2E | **Gate 6:** Implementado (Ruta C). Redirección de `/circulos` a `/alza-la-voz`. Tab 'Más' implementa la arquitectura overlay del hub secundario; acceso navegable activo para 2.1 y reserva estructural para 2.5 y 2.6, aún no funcionales. *(Nota: El cierre 'OK' es provisional hasta validación local de E2E y consolidación del hash real en cabecera).* |
| UI-NAV-002 | MUST | Dock altura 56px + safe area iOS | S01 → §9.2 → "Altura: 56px" (L1141) | OK | MEDIO | Golden screenshots | **Gate 5.9a (cerrado).** Estructura: `<nav pb-safe>` → `<div h-[56px]>` (zona útil) → items. Total exterior = 56px + env(safe-area-inset-bottom). `viewport-fit=cover` confirmado presente en `webapp/index.html`. CSS utility `.pb-safe { padding-bottom: env(safe-area-inset-bottom) }` en `index.css`. Verificable en dispositivo iOS real con notch; en desktop/emulador el inset es 0px y el dock mide exactamente 56px. Hash HEAD: `6e2ed35`. |
