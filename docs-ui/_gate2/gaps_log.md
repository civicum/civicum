...
|--------|-----------|---------------------|---------|----------------------|
| GAP-001 | Ruta 404: no existe ninguna pantalla ni ruta catch-all para URLs inválidas. **Gate 3:** se resuelve vía ADR de diseño (definir intención, copy y layout de 404), no esperando una fuente externa. | `04_NAVIGATION` (routing) y código `src/App.tsx` | ALTO | (A) Crear ruta catch-all con redirect a Dashboard, (B) Crear pantalla 404 personalizada con tono "Vecino Organizado" y CTA, (C) Pantalla 404 + sugerencias tipo "¿Buscabas…?" |
