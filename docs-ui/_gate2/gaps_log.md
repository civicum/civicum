...
| GAP_ID | Qué falta | Dónde debería estar | Impacto | Opciones para resolver | ADR Draft | Status |
|--------|-----------|---------------------|---------|----------------------|-----------|--------|
| GAP-001 | Ruta 404: no existe ninguna pantalla ni ruta catch-all para URLs inválidas. **Gate 3:** se resuelve vía ADR de diseño (definir intención, copy y layout de 404), no esperando una fuente externa. | `04_NAVIGATION` (routing) y código `src/App.tsx` | ALTO | (A) Crear ruta catch-all con redirect a Dashboard, (B) Crear pantalla 404 personalizada con tono CIVICUM (empático, no técnico — ref: UI-CNT-001) y CTA, (C) Pantalla 404 + sugerencias tipo "¿Buscabas…?" | [ADR-0004](../../adr-ui/ADR-0004-diseno-404.md) (ACCEPTED) | **DECIDED** |
...
- Los GAPs de **impacto ALTO** son: GAP-001 (404), GAP-005 (dark mode), GAP-006 (estados), GAP-007 (tipografía). **Todos DECIDED vía ADR ACCEPTED (2026-02-26).**
- GAP-005 (dark mode): pospuesto a Época 3+ vía ADR-0005 (ACCEPTED).
- GAP-007 (tipografía): migración a Nunito Sans + IBM Plex aprobada vía ADR-0007 (ACCEPTED). Implementación en Gate 5+.
