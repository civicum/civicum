...
import { BookOpen, Trophy, TrendingUp, ArrowRight, GraduationCap, Zap, Clock, Star } from 'lucide-react';
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-black">L0</span>
                <span className="text-azul-200 text-sm">Nivel Observador</span>
              </div>
              <div className="flex items-center gap-2 mt-3">
                <div className="w-48 h-2 bg-azul-400/40 rounded-full overflow-hidden">
                  <div className="h-full w-0 bg-white rounded-full" />
                </div>
                <span className="text-xs text-azul-200">0% completado</span>
              </div>
            </div>
            <div className="hidden sm:flex items-center gap-3">
              <div className="text-center">
                ...
                <p className="text-xs text-azul-200 mt-1">0 Insignias</p>
              </div>
              <div className="text-center">
                <div className="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center mx-auto">
                  <Star className="w-7 h-7 text-dorado-300" />
                </div>
                <p className="text-xs text-azul-200 mt-1">0 Karma</p>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {MODULOS_ACADEMIA.map((modulo) => (
            <Card
              key={modulo.titulo}
              className={`border shadow-sm hover:shadow-md transition-shadow ${modulo.color}`}
            >
              <CardContent className="p-5 space-y-3">
                ...
                <Button
                  ...
                  disabled
                >
                  Próximamente <ArrowRight className="ml-2 w-4 h-4" />
                </Button>
              </CardContent>
            </Card>
          ))}
      ...
      <Card className="border-dashed border-2 border-slate-300 bg-transparent">
        <CardContent className="p-8 text-center space-y-3">
          ...
          <p className="text-sm text-slate-500 max-w-md mx-auto">
            Cada módulo que completes sube tu Nivel de Credibilidad Cívica y
